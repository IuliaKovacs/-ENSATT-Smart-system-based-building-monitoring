...
void counterSysSTM() {
  ...
  switch (currentState) {
      case IDLE:
        ...
        if (enterDistanceValue < DISTANCE_THRESHOLD) {
          currentState = ENTER_DETECT;
          lastTransitionTime = millis();
        } else if (leaveDistanceValue < DISTANCE_THRESHOLD) {
      ...
      case ENTER_COMPLETE:
        if((leaveDistanceValue > DISTANCE_THRESHOLD) || (leaveDistanceValue == -1)){
          peopleCount++;
          Serial.println("Person ENTERED. Total: " + String(peopleCount));
          currentState = IDLE;
        }
        break;
      ...
      case LEAVE_COMPLETE:
        if(peopleCount > 0){
          if ((enterDistanceValue > DISTANCE_THRESHOLD) || (enterDistanceValue == -1)) {
            peopleCount--;
            Serial.println("Person EXITED. Total: " + String(peopleCount));
            currentState = IDLE;
          }
        } else {
          Serial.println("Error: The counter detected someone exiting, even though the room was already empty.");
          currentState = IDLE;
        }
        break;
    }
